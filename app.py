...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/keyariawHW5"
## Provided:
...
class UpdatePriorityForm(FlaskForm):
    newPriority = StringField("What is the new priority", validators=[Required()])
    submit = SubmitField('Update')
...
def delete(lst):
    ...
    lit = TodoList.query.filter_by(title=lst).first()
    print(lit)
    print('HEEEEEE')
